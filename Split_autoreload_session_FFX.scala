...
Inputs - User has to pass one argument - week number for which it will be filtered. 
...
3.) Assumption made here is that if the same tracking ID visits two different mastheads, it can be considered as the same session and is the path that they followed. We are not looking in detail at the value of referer and url to identify if the source was internal
...
2.) In case of addition of any masthead, add it to the list of OtherClickLocation file name value
...
object auto_reload {
  ...
  def main(args:Array[String]) {
    ...
    val conf = new SparkConf().setAppName("FFX_Sessionise")

    val sc = new SparkContext(conf)

    var SMHClicksLocation = "s3n://ffx-analytics/all-interactions/11-2014-smh*view-*.gz"
    var AGEClicksLocation = "s3n://ffx-analytics/all-interactions/11-2014-theage*view-*.gz"
    var OtherClicksLocation = "s3n://ffx-analytics/all-interactions/11-2014-{drive,domain,traveller}*view*"

    //Reading as a textfile
    var read_text_smh = sc.textFile(SMHClicksLocation,1000)

    var clicks_smh: RDD[Click] = (read_text_smh
...
//Get the index views from the raw click data
    var trackid_timestamp_asset = week_clicks
	.map(c=>(c.getOrElse("trackingId",""),c.getOrElse("timestamp",""),c.getOrElse("assetType","").toString,c.getOrElse("assetCategory","").toString))
	.filter(t=>(t._3 == "Index" && t._4 == "The Sydney Morning Herald"))
...
//Smoothen the exponential curve and flag based on the probability equations and difference from the peak of 5 min whether a click is auto-reload
	...
	.map(t=>(t,(if(t._3.toLong>0 && t._4.toLong>0) rescale * math.exp(-math.pow(t._4.toDouble/120000.0,2))
	      ...
	      else 0))).map(t=>(t,if (t._2==0) 0 else if(t._2>Random.nextDouble()) 1 else 0))
...
//Join the auto-reload clicks to the original data and filter out the auto-reloads
    var after_autoreload_smh = week_clicks
	.map(c=>((c.getOrElse("trackingId",""),c.getOrElse("timestamp","")),c)).leftOuterJoin(autoreload)
	.filter(t=>(t._2._2.getOrElse("") == ""))
	.map(c=>(c._2._1))
    //after_autoreload.take(10).foreach(println)

        //Reading as a textfile
    var read_text_age = sc.textFile(AGEClicksLocation,1000)

    var clicks_age: RDD[Click] = (read_text_age
      .filter((l: String) => !(l contains "00000000-0000-0000-0000-000000000000"))
      .map(expandRecord(_))
    )

//Filter for the relevant week
    week_clicks = clicks_age
	.map(c=>(toWeek(c.getOrElse("timestamp","").toString.toLong),c))
	.filter(c=>(c._1 == wk))
	.map(c=>(c._2))

//Get the index views from the raw click data
    trackid_timestamp_asset = week_clicks
	.map(c=>(c.getOrElse("trackingId",""),c.getOrElse("timestamp",""),c.getOrElse("assetType","").toString,c.getOrElse("assetCategory","").toString))
	.filter(t=>(t._3 == "Index" && t._4 == "The Age"))

//Get assets ordered by timestamp for each tracking ID
    groupby_trackid = trackid_timestamp_asset
	.map(t=>(t._1,(t._2)))
	.groupByKey()
	.map(t=>(t._1,convert_to_pair(t._2.toList.sorted)))

//Smoothen the exponential curve and flag based on the probability equations and difference from the peak of 5 min whether a click is auto-reload
    trackingid_timestamp_asset = groupby_trackid
	.flatMap(t=>(for(x<-t._2) yield(t._1,x))).map(t=>(t._1, t._2._1, t._2._2.toString.toLong - t._2._1.toString.toLong, t._2._2.toString.toLong - t._2._1.toString.toLong - 300000L))
	.map(t=>(t,(if(t._3.toLong>0 && t._4.toLong>0) rescale * math.exp(-math.pow(t._4.toDouble/120000.0,2))
	        else if(t._3.toLong>0) rescale * math.exp(-math.pow(t._4.toDouble/140000.0,2))
	        else 0))).map(t=>(t,if (t._2==0) 0 else if(t._2>Random.nextDouble()) 1 else 0))

//Based on the mix-model results from David, flag if a click is auto-reload
    autoreload = trackingid_timestamp_asset
	.filter(t=>(t._2 == 1 && Random.nextDouble()>0.14))
	.map(t=>((t._1._1._1, t._1._1._2), 1))

//Join the auto-reload clicks to the original data and filter out the auto-reloads
      var after_autoreload_age = week_clicks
	.map(c=>((c.getOrElse("trackingId",""),c.getOrElse("timestamp","")),c))
	.leftOuterJoin(autoreload)
	.filter(t=>(t._2._2.getOrElse("") == ""))
	.map(c=>(c._2._1))

//Reading as a textfile the remaining non-auto-reload mastheads
      var read_text_others = sc.textFile(OtherClicksLocation,1000)

      var clicks_others: RDD[Click] = (read_text_others
        .filter((l: String) => !(l contains "00000000-0000-0000-0000-000000000000"))
        .map(expandRecord(_))
      )

//Filter for the relevant week
      var week_clicks_others = clicks_others
	.map(c=>(toWeek(c.getOrElse("timestamp","").toString.toLong),c))
	.filter(c=>(c._1 == wk))
	.map(c=>(c._2))

//Append all the auto-reload removed clicks
      var after_autoreload = after_autoreload_smh ++ after_autoreload_age ++ week_clicks_others
...
//Add the sessionID and referer to the click data using updated statement
	...
	.map(c=> (c._1.updated("finalReferer",c._2.toString)))
        ...
        var save_file = masthead.toString + "_wk" + wk + "_sessionised_mon"
        var save = sessionised_clicks
        .map(serialise(_))
        .repartition(50)

        save.take(5).foreach(println)
